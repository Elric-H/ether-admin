...
public class WordProducer implements Producer {
    ...
    private void buildTableCellStyle(XWPFTable table, XWPFTableCell tableCell, Cell cell, int rowNumber, int columnNumber) {
        ...
        boolean underline = style.getUnderline() != null && style.getUnderline();
        ...
        int endRow = rowNumber, endCol = columnNumber;
        if (colSpan > 0) {
        ...
        if (columnNumber != endCol) {
            if (rowSpan > 0) {
                for (int i = rowNumber; i <= endRow; i++) {
                    mergeCellsHorizontal(table, i, columnNumber, endCol);
                }
            } else {
                mergeCellsHorizontal(table, rowNumber, columnNumber, endCol);
            }
        }
        if (rowNumber != endRow) {
            if (colSpan > 0) {
                for (int i = columnNumber; i <= endCol; i++) {
                    mergeCellsVertically(table, i, rowNumber, endRow);
                }
            } else {
                mergeCellsVertically(table, columnNumber, rowNumber, endRow);
            }
        }
    }
