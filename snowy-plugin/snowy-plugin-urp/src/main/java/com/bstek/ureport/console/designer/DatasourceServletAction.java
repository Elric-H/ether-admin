    ...
    public void previewData(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (ProcedureUtils.isProcedure(sql)) {
                ...
                // 防止非查询SQL
                if(!StrUtil.startWithIgnoreCase(sql, "SELECT")) {
                    throw new CommonException("非法的SQL语句");
                }
                // 动态SQL
                AtomicReference<String> resultSql = new AtomicReference<>(sql);
                map.forEach((key, value) -> resultSql.set(StrUtil.replace(resultSql.get(), StrUtil.COLON + key,
                        "\"" + Convert.toStr(value) + "\"")));
                List<JSONObject> mapList = dynamicSqlExecutorMapper.dynamicSelectSqlExecutor(resultSql.get());
                list = mapList.stream().map(jsonObject -> {
