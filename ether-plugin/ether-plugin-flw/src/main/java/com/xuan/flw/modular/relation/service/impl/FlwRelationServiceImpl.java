 ...
 */
package com.xuan.flw.modular.relation.service.impl;
...
import com.xuan.flw.modular.relation.entity.FlwRelation;
import com.xuan.flw.modular.relation.mapper.FlwRelationMapper;
import com.xuan.flw.modular.relation.service.FlwRelationService;
...
public class FlwRelationServiceImpl extends ServiceImpl<FlwRelationMapper, FlwRelation> implements FlwRelationService {
    ...
    public void saveRelation(String objectId, String targetId, String category, String extJson, boolean clear) {
        // 是否需要先删除关系
        if(clear) {
            this.remove(new LambdaQueryWrapper<FlwRelation>().eq(FlwRelation::getObjectId, objectId)
                    .eq(FlwRelation::getCategory, category));
        }
        FlwRelation flwRelation = new FlwRelation();
        flwRelation.setObjectId(objectId);
        flwRelation.setTargetId(targetId);
        flwRelation.setCategory(category);
        flwRelation.setExtJson(extJson);
        this.save(flwRelation);
    }
    ...
    public void saveRelationBatch(String objectId, List<String> targetIdList, String category, List<String> extJsonList, boolean clear) {
        ...
        for(int i = 0; i < targetIdList.size(); i++) {
            FlwRelation flwRelation = new FlwRelation();
            flwRelation.setObjectId(objectId);
            flwRelation.setTargetId(targetIdList.get(i));
            flwRelation.setCategory(category);
            if(ObjectUtil.isNotEmpty(extJsonList)) {
                flwRelation.setExtJson(extJsonList.get(i));
            }
            flwRelationList.add(flwRelation);
        }
        if(ObjectUtil.isNotEmpty(flwRelationList)) {
            this.saveBatch(flwRelationList);
        }
    }
    ...
    @Override
    public List<FlwRelation> getRelationListByObjectId(String objectId) {
        return this.getRelationListByObjectIdAndCategory(objectId, null);
    }

    @Override
    public List<FlwRelation> getRelationListByObjectIdList(List<String> objectIdList) {
        return this.getRelationListByObjectIdListAndCategory(objectIdList, null);
    }

    @Override
    public List<FlwRelation> getRelationListByObjectIdAndCategory(String objectId, String category) {
        LambdaQueryWrapper<FlwRelation> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(FlwRelation::getObjectId, objectId);
        if(ObjectUtil.isNotEmpty(category)) {
            lambdaQueryWrapper.eq(FlwRelation::getCategory, category);
        }
        return this.list(lambdaQueryWrapper);
    }

    @Override
    public List<FlwRelation> getRelationListByObjectIdListAndCategory(List<String> objectIdList, String category) {
        LambdaQueryWrapper<FlwRelation> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.in(FlwRelation::getObjectId, objectIdList);
        if(ObjectUtil.isNotEmpty(category)) {
            lambdaQueryWrapper.eq(FlwRelation::getCategory, category);
        }
        return this.list(lambdaQueryWrapper);
    ...
    @Override
    public List<FlwRelation> getRelationListByTargetId(String targetId) {
        return this.getRelationListByTargetIdAndCategory(targetId, null);
    }

    @Override
    public List<FlwRelation> getRelationListByTargetIdList(List<String> targetIdList) {
        return this.getRelationListByTargetIdListAndCategory(targetIdList, null);
    }

    @Override
    public List<FlwRelation> getRelationListByTargetIdAndCategory(String targetId, String category) {
        LambdaQueryWrapper<FlwRelation> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(FlwRelation::getTargetId, targetId);
        if(ObjectUtil.isNotEmpty(category)) {
            lambdaQueryWrapper.eq(FlwRelation::getCategory, category);
        }
        return this.list(lambdaQueryWrapper);
    ...
    @Override
    public List<FlwRelation> getRelationListByTargetIdListAndCategory(List<String> targetIdList, String category) {
        LambdaQueryWrapper<FlwRelation> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.in(FlwRelation::getTargetId, targetIdList);
        if(ObjectUtil.isNotEmpty(category)) {
            lambdaQueryWrapper.eq(FlwRelation::getCategory, category);
        }
        return this.list(lambdaQueryWrapper);
    ...
    public List<String> getRelationTargetIdListByObjectIdAndCategory(String objectId, String category) {
        return this.getRelationListByObjectIdAndCategory(objectId, category).stream()
                .map(FlwRelation::getTargetId).collect(Collectors.toList());
    }
    ...
    public List<String> getRelationObjectIdListByTargetIdAndCategory(String targetId, String category) {
        return this.getRelationListByTargetIdAndCategory(targetId, category).stream()
                .map(FlwRelation::getObjectId).collect(Collectors.toList());
    }
