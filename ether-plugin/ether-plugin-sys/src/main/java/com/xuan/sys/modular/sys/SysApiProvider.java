package com.xuan.sys.modular.sys;
...
import com.xuan.common.util.CommonAvatarUtil;
import com.xuan.common.util.CommonCryptogramUtil;
import com.xuan.sys.api.SysApi;
import com.xuan.sys.core.enums.SysBuildInEnum;
import com.xuan.sys.modular.org.entity.SysOrg;
import com.xuan.sys.modular.org.enums.SysOrgCategoryEnum;
import com.xuan.sys.modular.org.mapper.SysOrgMapper;
import com.xuan.sys.modular.org.service.SysOrgService;
import com.xuan.sys.modular.position.entity.SysPosition;
import com.xuan.sys.modular.position.enums.SysPositionCategoryEnum;
import com.xuan.sys.modular.position.mapper.SysPositionMapper;
import com.xuan.sys.modular.position.service.SysPositionService;
import com.xuan.sys.modular.relation.entity.SysRelation;
import com.xuan.sys.modular.relation.enums.SysRelationCategoryEnum;
import com.xuan.sys.modular.relation.service.SysRelationService;
import com.xuan.sys.modular.resource.entity.SysButton;
import com.xuan.sys.modular.resource.entity.SysMenu;
import com.xuan.sys.modular.resource.entity.SysModule;
import com.xuan.sys.modular.resource.enums.SysResourceCategoryEnum;
import com.xuan.sys.modular.resource.enums.SysResourceMenuTypeEnum;
import com.xuan.sys.modular.resource.mapper.SysButtonMapper;
import com.xuan.sys.modular.resource.mapper.SysMenuMapper;
import com.xuan.sys.modular.resource.mapper.SysModuleMapper;
import com.xuan.sys.modular.resource.service.SysButtonService;
import com.xuan.sys.modular.resource.service.SysMenuService;
import com.xuan.sys.modular.resource.service.SysModuleService;
import com.xuan.sys.modular.role.entity.SysRole;
import com.xuan.sys.modular.role.enums.SysRoleCategoryEnum;
import com.xuan.sys.modular.role.enums.SysRoleDataScopeCategoryEnum;
import com.xuan.sys.modular.role.mapper.SysRoleMapper;
import com.xuan.sys.modular.role.service.SysRoleService;
import com.xuan.sys.modular.user.entity.SysUser;
import com.xuan.sys.modular.user.enums.SysUserStatusEnum;
import com.xuan.sys.modular.user.mapper.SysUserMapper;
import com.xuan.sys.modular.user.service.SysUserService;
...
public class SysApiProvider implements SysApi {
    ...
    public void removeTenDataForCategoryId(String tenId) {
        // ==============移除租户初始化的组织=================
        sysOrgMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysOrg>().eq(SysOrg::getTenantId, tenId));

        // ==============移除租户初始化的职位=================
        sysPositionMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysPosition>().eq(SysPosition::getTenantId, tenId));

        // ==============移除租户初始化的模块=================
        sysModuleMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysModule>().eq(SysModule::getTenantId, tenId));

        // ==============移除租户初始化的菜单=================
        sysMenuMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysMenu>().eq(SysMenu::getTenantId, tenId));

        // ==============移除租户初始化的按钮=================
        sysButtonMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysButton>().eq(SysButton::getTenantId, tenId));

        // ==============移除租户初始化的角色=================
        sysRoleMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysRole>().eq(SysRole::getTenantId, tenId));

        // ==============移除租户初始化的用户=================
        sysUserMapper.deleteIgnoreInterceptor(new LambdaQueryWrapper<SysUser>().eq(SysUser::getTenantId, tenId));
    }
